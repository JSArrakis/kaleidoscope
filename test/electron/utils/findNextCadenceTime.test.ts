...
import { findNextCadenceTime } from "../../../src/electron/utils/common";
...
describe("findNextCadenceTime", () => {
  it("should return next :30 mark when current time has minutes < 30", () => {
    ...
    const currentTime = getUnixTime(
      parse("2025-01-01 14:15:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
    const expectedTime = getUnixTime(
      parse("2025-01-01 14:30:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
  ...
  it("should return same time when at exact :00 mark", () => {
    ...
    const currentTime = getUnixTime(
      parse("2025-01-01 14:00:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
    const expectedTime = getUnixTime(
      parse("2025-01-01 14:00:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
  ...
  it("should return next :00 mark when current time has minutes >= 30", () => {
    ...
    const currentTime = getUnixTime(
      parse("2025-01-01 14:45:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
    const expectedTime = getUnixTime(
      parse("2025-01-01 15:00:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
  ...
  it("should find next cadence when close but not exact", () => {
    ...
    const currentTime = getUnixTime(
      parse("2025-01-01 14:00:01", "yyyy-MM-dd HH:mm:ss", new Date())
    );
    const expectedTime = getUnixTime(
      parse("2025-01-01 14:30:00", "yyyy-MM-dd HH:mm:ss", new Date())
    );
