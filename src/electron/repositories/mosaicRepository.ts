...
export class MosaicRepository {
  ...
  create(mosaic: Mosaic): Mosaic {
    const transaction = this.db.transaction(() => {
      ...
      stmt.run(
        mosaic.mosaicId,
        mosaic.facetId,
        JSON.stringify(mosaic.musicalGenres),
        mosaic.name || null,
        mosaic.description || null
      );
    });
  ...
  findByMosaicId(mosaicId: string): Mosaic | null {
    ...
    const row = stmt.get(mosaicId) as any;
    if (!row) return null;
  ...
  findByFacetIds(facetIds: string[]): Mosaic[] {
    ...
    const rows = stmt.all(...facetIds) as any[];
    return rows.map((row) => this.mapRowToMosaic(row));
  }
  ...
  findByMusicalGenres(musicalGenres: string[]): Mosaic[] {
    ...
    return allMosaics.filter((mosaic) =>
      musicalGenres.some((genre) => mosaic.musicalGenres.includes(genre))
    );
  }
  ...
  findByAllMusicalGenres(musicalGenres: string[]): Mosaic[] {
    ...
    const allMosaics = this.findAll();
    return allMosaics.filter((mosaic) =>
      musicalGenres.every((genre) => mosaic.musicalGenres.includes(genre))
    );
  }
   ...
   */
  update(mosaicId: string, mosaic: Mosaic): Mosaic | null {
    const transaction = this.db.transaction(() => {
      const stmt = this.db.prepare(`
        UPDATE mosaics 
   ...
   */
  private mapRowToMosaic(row: any): Mosaic {
    let musicalGenres: string[] = [];
    try {
