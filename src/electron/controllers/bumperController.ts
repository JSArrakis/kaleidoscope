...
export async function createBumper(
  ...
  try {
    ...
    const bumperWithDuration: Bumper = {
      ...bumper,
...
export function getAllBumpers(): Bumper[] {
  return bumperRepository.findAll();
}

export function getBumper(mediaItemId: string): Bumper | null {
  if (!mediaItemId) {
...
export function deleteBumper(mediaItemId: string): {
  message: string;
  status: number;
} {
  try {
