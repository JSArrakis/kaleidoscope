 ...
 */
export function canFacetsWalk(percentile: number = 80): boolean {
  ...
  for (const facet of allFacets) {
    ...
    for (const relationship of facet.facetRelationships) {
      ...
      for (const otherFacet of allFacets) {
        if (otherFacet.facetId === facet.facetId) continue;

        // Connect if they share a genre or aesthetic
        const shareGenre =
          (facet.genre?.tagId === otherFacet.genre?.tagId &&
            facet.genre?.tagId) ||
        ...
        const shareAesthetic =
          (facet.aesthetic?.tagId === otherFacet.aesthetic?.tagId &&
  ...
  for (const facet of allFacets) {
    if (visited.has(facet.facetId)) continue;
