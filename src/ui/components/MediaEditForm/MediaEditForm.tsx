...
interface MediaEditFormProps {
  ...
  onSave: (item: Movie | Show) => void;
  onCancel: (item: Movie | Show) => void;
}
...
const MediaEditForm: FC<MediaEditFormProps> = ({
  ...
  const [tagObjectList, setTagObjectList] = useState<Tag[]>(item.tags);
  ...
  }, ["episodes" in item ? item.episodes : undefined]);
  ...
  const onAddEpisode = async () => {
    ...
    if (filePaths.length > 0) {
      const newEpisodes = filePaths.map(
        (episodePath: string) =>
          ({
            mediaItemId: normalizeItem(episodePath),
            showItemId: itemType === "show" ? (item as Show).mediaItemId : "",
            title: "",
            path: episodePath,
            season: "",
            episode: "",
  ...
  const handleSave = () => {
    ...
    let updatedItem: Movie | Show = {
      ...item,
      title,
      alias,
      imdb,
      tags: newTags,
      ...(itemType === "movie" && { isHolidayExclusive }),
    } as Movie | Show;

    if ("episodes" in item && item.episodes) {
      (updatedItem as Show).episodes = episodeList as Episode[];
    }
