...
export class FacetRepository {
  ...
  selectValidRandomFacetCombo(
    ...
    try {
      ...
      const result = stmt.get(durationWindow, durationWindow) as
        | {
            facetId: string;
            genre_name: string;
      ...
      if (result) {
        console.log(
          `[Facet Repository] Selected valid facet via SQL: ${result.genre_name}/${result.aesthetic_name} (verified with media)`,
        );
        ...
        console.log(
          '[Facet Repository] No valid facets found with available media via SQL',
        );
        return null;
      }
    } catch (error) {
      console.error(
        '[Facet Repository] Error executing facet selection query:',
        error,
      );
      return null;
    }
  ...
  private mapRowToFacet(row: any): Facet {
    ...
    const genreTag = new Tag(
      row.genreTagId,
      row.genreName,
      row.genreType as TagType,
      row.genreSeasonStartDate ? [row.genreSeasonStartDate] : undefined,
      undefined, // exclusionTags
      row.genreSeasonStartDate,
      row.genreSeasonEndDate,
      row.genreExplicitlyHoliday,
      row.genreSequence,
    );

    // Create aesthetic Tag object
    const aestheticTag = new Tag(
      row.aestheticTagId,
      row.aestheticName,
      row.aestheticType as TagType,
      row.aestheticSeasonStartDate ? [row.aestheticSeasonStartDate] : undefined,
      undefined, // exclusionTags
      row.aestheticSeasonStartDate,
      row.aestheticSeasonEndDate,
      row.aestheticExplicitlyHoliday,
      row.aestheticSequence,
    );

    return new Facet(row.facetId, genreTag, aestheticTag, relationships);
  }
}
