...
exports.constructStream = constructStream;
exports.getInitialProceduralTimepoint = getInitialProceduralTimepoint;
exports.setInitialBlockDuration = setInitialBlockDuration;
exports.getStagedStream = getStagedStream;
exports.setProceduralTags = setProceduralTags;
exports.evaluateStreamEndTime = evaluateStreamEndTime;
exports.compareSelectedEndTime = compareSelectedEndTime;
exports.getScheduledMedia = getScheduledMedia;
exports.getInjectedMovies = getInjectedMovies;
exports.getMovie = getMovie;
exports.getBlock = getBlock;
exports.assignBlockEpisodes = assignBlockEpisodes;
exports.selectIntelligentStartingTags = selectIntelligentStartingTags;
const movie_1 = require("../models/movie");
const moment = require('moment');
const mediaTypes_1 = require("../models/enum/mediaTypes");
const selectedMedia_1 = require("../models/selectedMedia");
const proceduralEngine_1 = require("./proceduralEngine");
const show_1 = require("../models/show");
const mediaBlock_1 = require("../models/mediaBlock");
const streamTypes_1 = require("../models/enum/streamTypes");
const progressionManager_1 = require("./progressionManager");
const facetRepository_1 = require("../repositories/facetRepository");
const tagsRepository_1 = require("../repositories/tagsRepository");
const refract_1 = require("../prisms/refract");
const holidayService_1 = require("./holidayService");
// Helper functions using existing repository logic
function selectRandomFacetCombo() {
    ...
    const selectedFacet = (0, refract_1.pickFacet)(allFacets);
    if (!selectedFacet) {
        return null;
    }
    return {
...
function findMediaWithFacet(facet) {
    // Use existing gatherCandidatesForFacet function to find matching media
    const candidates = [];
    (0, refract_1.gatherCandidatesForFacet)(facet).then(media => {
    ...
    return (0, refract_1.pickMediaCandidate)(candidates);
}
...
function constructStream(streamType, rightNow, // Time should always be passed in, no default calculation
cadence = false, firstMedia) {
    ...
    switch (streamType) {
        case streamTypes_1.StreamType.Cont:
            return constructContinuousStream(rightNow, cadence, activeHolidayTags, firstMedia);
...
function constructContinuousStream(rightNow, cadence, activeHolidayTags, firstMedia) {
    ...
    let selectedFirstMedia;
    if (firstMedia) {
        selectedFirstMedia = firstMedia;
        console.log(`[Continuous Stream] Using provided first media: ${firstMedia.title} (${firstMedia.mediaItemId})`);
    }
    else {
        // This path is used when called from background service
        console.log('[Continuous Stream] No first media provided, selecting via refract system');
        ...
        console.log(`[Continuous Stream] Selected first media: ${selectedFirstMedia.title} (${selectedFirstMedia.mediaItemId})`);
    }
    ...
    console.log(`[Continuous Stream] Creating main media block for: ${selectedFirstMedia.title}`);
    // Handle different media types for MediaBlock
    let featureMedia;
    let mediaDuration = 0;
    if (selectedFirstMedia.type === mediaTypes_1.MediaType.Show) {
        // It's a Show - select first episode
        const episodes = selectedFirstMedia.episodes;
        if (episodes && episodes.length > 0) {
            featureMedia = episodes[0];
        ...
        featureMedia = selectedFirstMedia || show_1.Episode;
        mediaDuration = featureMedia.duration;
    }
    const firstMediaBlock = new mediaBlock_1.MediaBlock([], // buffer (no buffer for main media block)
    ...
    console.log(`[Continuous Stream] Created ${streamBlocks.length} media blocks`);
    return [streamBlocks, ''];
}
...
//   return [streamBlocks, ''];
// }
function getInitialProceduralTimepoint(rightNow, stagedMedia) {
    let error = '';
    ...
    let firstTimePoint = stagedMedia.endTime;
...
function setInitialBlockDuration(interval, firstProceduralDuration) {
...
function getStagedStream(rightNow, config, args, stagedMedia, media, streamType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let selectedMedia = [];
        let prevMovies = [];
        stagedMedia.scheduledMedia.forEach(item => prevMovies.push(item.media));
        if (initialProceduralBlockDuration > 0) {
            let firstProceduralBlock = yield (0, proceduralEngine_1.getProceduralBlock)(args, stagedMedia, media, prevMovies, initialProceduralBlockDuration, rightNow + preMediaDuration, streamType);
        ...
        for (let index = 0; index < stagedMedia.scheduledMedia.length; index++) {
            ...
            if (index < stagedMedia.scheduledMedia.length - 1) {
                let procDuration = stagedMedia.scheduledMedia[index + 1].time -
                    stagedMedia.scheduledMedia[index].time -
                    stagedMedia.scheduledMedia[index].duration;
                if (procDuration > 0) {
                    let intermediateProcBlock = yield (0, proceduralEngine_1.getProceduralBlock)(args, stagedMedia, media, prevMovies, procDuration, stagedMedia.scheduledMedia[index].time +
                        stagedMedia.scheduledMedia[index].duration, streamType);
        ...
        if (stagedMedia.scheduledMedia.length > 0) {
            ...
            if (endProcDuration > 0) {
                let endProcBlock = yield (0, proceduralEngine_1.getProceduralBlock)(args, stagedMedia, media, prevMovies, endProcDuration, stagedMedia.scheduledMedia[stagedMedia.scheduledMedia.length - 1].time +
...
function setProceduralTags(options, stagedMedia) {
    if (options.MultiTags.length === 0 && options.Tags.length === 0) {
        ...
        if (uniquetags.length === 0) {
            console.log('📋 No tags found from scheduled/injected media - using intelligent tag selection');
...
function evaluateStreamEndTime(options, scheduledMedia) {
    let endTime = moment().startOf('day').add(1, 'days').unix();
    let error = '';
    if (options.EndTime) {
...
function compareSelectedEndTime(endTime, scheduledMedia) {
    let error = '';
...
function getScheduledMedia(media, args) {
    let selectedMedia = [];
...
function getInjectedMovies(options, movies) {
    let selectedMedia = [];
    let error = '';
    if (options.Movies !== undefined) {
        options.Movies.filter((str) => !str.includes('::')).forEach((str) => {
...
function getMovie(loadTitle, movieList, time) {
    let selectedMedia = new selectedMedia_1.SelectedMedia(new movie_1.Movie('', '', '', '', [], '', 0, 0, mediaTypes_1.MediaType.Movie, []), '', mediaTypes_1.MediaType.Movie, 0, 0, []);
    ...
    let selectedMovie = movieList.find(movie => movie.mediaItemId === loadTitle);
    ...
    selectedMedia = new selectedMedia_1.SelectedMedia(selectedMovie, '', mediaTypes_1.MediaType.Movie, time, selectedMovie.durationLimit, selectedMovie.tags);
...
function getBlock(loadTitle, media, time, args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let selectedBlock = media.blocks.find(block => block.mediaItemId === loadTitle);
        ...
        yield assignBlockEpisodes(args, selectedBlock, media.shows);
        return new selectedMedia_1.SelectedMedia(selectedBlock, '', mediaTypes_1.MediaType.Block, time, selectedBlock.durationLimit, selectedBlock.tags);
    });
}
function assignBlockEpisodes(args, block, shows) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a;
        ...
        for (const blockShow of block.shows) {
            ...
            let selectedShow = shows.filter(item => item.mediaItemId === blockShow.mediaItemId)[0];
            // Get the episode number that the show should be on based on the progression of the show
            let episodeNums = yield (0, progressionManager_1.ManageShowProgression)(selectedShow, 1, args, streamTypes_1.StreamType.Block, block.title);
            let episodeNum = episodeNums[0];
            // Get the episode that matches the episode number from the progression
            blockShow.episode = (_a = selectedShow.episodes) === null || _a === void 0 ? void 0 : _a.filter(ep => ep.episodeNumber === episodeNum)[0];
        }
    });
}
/**
 ...
 */
function hasMediaWithTag(media, tag) {
    const tagName = tag.name;
    // Check movies
    if (media.movies &&
        media.movies.some((movie) => movie.tags &&
            movie.tags.some((t) => (typeof t === 'string' ? t : t.name) === tagName))) {
        return true;
    }
    // Check shows
    if (media.shows &&
        media.shows.some((show) => show.tags &&
            show.tags.some((t) => (typeof t === 'string' ? t : t.name) === tagName))) {
        return true;
    }
    // Check music
    if (media.music &&
        media.music.some((music) => music.tags &&
            music.tags.some((t) => (typeof t === 'string' ? t : t.name) === tagName))) {
        return true;
    }
    // Check shorts
    if (media.shorts &&
        media.shorts.some((short) => short.tags &&
            short.tags.some((t) => (typeof t === 'string' ? t : t.name) === tagName))) {
        return true;
    }
 ...
 */
function getRandomDefaultGenres() {
    const { getMedia } = require('./mediaService');
    const { tagRepository } = require('../repositories/tagsRepository');
    console.log('🎲 Using pure random genre selection - no time-based or other preferences applied');
    console.log('📝 Future enhancement: User-configurable programming preferences (time-based, mood-based, etc.)');
    ...
    console.log(`📚 Found ${availableGenreTags.length} total genre tags in database`);
    // Get current media to validate against
    const media = getMedia();
    // Find genre tags that actually have associated media
    const validGenreTags = availableGenreTags.filter((tag) => hasMediaWithTag(media, tag));
    if (validGenreTags.length === 0) {
        console.log('⚠️  No genre tags found with associated media - returning empty array');
        return [];
    }
    console.log(`✅ Found ${validGenreTags.length} genre tags with associated media`);
    ...
    selectedGenres.push(...shuffledTags.slice(0, Math.min(numGenresToSelect, shuffledTags.length)));
    console.log(`🎯 Selected ${selectedGenres.length} random genres with verified media: ${selectedGenres.map((g) => g.name).join(', ')}`);
    console.log(`🎲 Random seed used: ${randomSeed} (ensures different selection each time)`);
 ...
 */
function getHolidayComplementaryGenres(holidays) {
    const complementaryGenres = [];
    console.log('🎄 Applying holiday-specific genre preferences (not general programming preferences)');
    // Holiday-specific genre mapping - this is contextually appropriate unlike general time preferences
    holidays.forEach((holiday) => {
        var _a;
        const holidayName = ((_a = holiday.name) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
 ...
 */
function selectIntelligentStartingTags() {
    ...
    if (activeHolidays && activeHolidays.length > 0) {
        ...
        console.log(`🎄 Active holidays detected: ${activeHolidays.map((h) => h.name).join(', ')}`);
    ...
    console.log(`🎲 No active holidays found. Using random genre selection for continuous stream`);
    selectedTags = getRandomDefaultGenres();
    console.log(`🎬 Selected ${selectedTags.length} random genres for continuous stream`);
    return selectedTags;
}
