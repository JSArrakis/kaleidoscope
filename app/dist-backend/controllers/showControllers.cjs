...
async function createShowHandler(req, res) {
...
async function updateShowHandler(req, res) {
    // Check for validation errors
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        res.status(400).json({ errors: errors.array() });
...
async function getShowHandler(req, res) {
    // Check for validation errors
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        res.status(400).json({ errors: errors.array() });
...
async function getAllShowsHandler(req, res) {
    const shows = showRepository_1.showRepository.findAll();
    res.status(200).json(shows);
    return;
}
async function transformShowFromRequest(show) {
