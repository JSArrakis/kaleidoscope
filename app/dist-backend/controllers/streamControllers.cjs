...
const streamMan = __importStar(require("../services/streamManager.cjs"));
const vlcService = __importStar(require("../services/vlcService.cjs"));
const streamTypes_1 = require("../models/enum/streamTypes.cjs");
const envMan = __importStar(require("../services/environmentManager.cjs"));
const streamRequest_1 = require("../models/streamRequest.cjs");
const utilities_1 = require("../utils/utilities.cjs");
const configService_1 = require("../config/configService.cjs");
const mediaService_1 = require("../services/mediaService.cjs");
const envConfig_1 = require("../db/envConfig.cjs");
async function continuousStreamHandler(req, res) {
    try {
        let streamError = '';
        ...
        streamError = await streamMan.initializeStream((0, configService_1.getConfig)(), contRequest, (0, mediaService_1.getStreamType)(), true);
        if (streamError !== '') {
...
async function adHocStreamHandler(req, res) {
    ...
    // Get environment config from DB and set it in the environment manager
    let env = await (0, envConfig_1.getEnvConfig)((0, utilities_1.keyNormalizer)(adhocRequest.Env));
    if (env[1] !== '') {
    ...
    await vlcService.initializeVLCService(streamMan.getContinuousStreamArgs().Password);
    await streamMan.initializeStream((0, configService_1.getConfig)(), adhocRequest, (0, mediaService_1.getStreamType)());
...
async function streamStatusHandler(req, res) {
    try {
...
async function stopStreamHandler(req, res) {
    try {
        if (!streamMan.isContinuousStream()) {
            res.status(400).json({
