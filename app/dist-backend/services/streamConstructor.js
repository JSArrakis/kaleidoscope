...
async function getStagedStream(rightNow, config, args, stagedMedia, media, streamType) {
    let error = '';
    ...
    if (initialProceduralBlockDuration > 0) {
        let firstProceduralBlock = await (0, proceduralEngine_1.getProceduralBlock)(args, stagedMedia, media, prevMovies, initialProceduralBlockDuration, rightNow + preMediaDuration, streamType);
    ...
    for (let index = 0; index < stagedMedia.scheduledMedia.length; index++) {
        ...
        if (index < stagedMedia.scheduledMedia.length - 1) {
            ...
            if (procDuration > 0) {
                let intermediateProcBlock = await (0, proceduralEngine_1.getProceduralBlock)(args, stagedMedia, media, prevMovies, procDuration, stagedMedia.scheduledMedia[index].time +
    ...
    if (stagedMedia.scheduledMedia.length > 0) {
        ...
        if (endProcDuration > 0) {
            let endProcBlock = await (0, proceduralEngine_1.getProceduralBlock)(args, stagedMedia, media, prevMovies, endProcDuration, stagedMedia.scheduledMedia[stagedMedia.scheduledMedia.length - 1].time +
...
async function getBlock(loadTitle, media, time, args) {
...
async function assignBlockEpisodes(args, block, shows) {
    ...
    for (const blockShow of block.shows) {
        ...
        let episodeNums = await (0, progressionManager_1.ManageShowProgression)(selectedShow, 1, args, streamTypes_1.StreamType.Block, block.title);
        let episodeNum = episodeNums[0];
        // Get the episode that matches the episode number from the progression
        blockShow.episode = selectedShow.episodes?.filter(ep => ep.episodeNumber === episodeNum)[0];
    }
}
/**
