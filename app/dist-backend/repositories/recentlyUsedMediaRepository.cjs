...
exports.recentlyUsedMediaRepository = exports.RecentlyUsedMediaRepository = void 0;
const sqlite_1 = require("../db/sqlite.cjs");
const recentlyUsedMedia_1 = require("../models/recentlyUsedMedia.cjs");
class RecentlyUsedMediaRepository {
    get db() {
        return (0, sqlite_1.getDB)();
    }
    // Get table name for media type
    getTableName(mediaType) {
        switch (mediaType) {
            case 'commercial':
    ...
    recordUsage(mediaItemId, mediaType, usageContext, streamSessionId, customExpirationDate) {
        const usedAt = new Date();
        const expiresAt = customExpirationDate ||
            recentlyUsedMedia_1.RecentlyUsedMedia.getDefaultExpirationDate(usageContext);
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        const result = stmt.run(mediaItemId, usageContext, streamSessionId || null, usedAt.toISOString(), expiresAt.toISOString());
        return new recentlyUsedMedia_1.RecentlyUsedMedia(mediaItemId, mediaType, usageContext, streamSessionId, usedAt, expiresAt, Number(result.lastInsertRowid));
    }
    // Get recently used media IDs for a specific media type and context
    getRecentlyUsedMediaIds(mediaType, usageContext, hoursBack) {
        const tableName = this.getTableName(mediaType);
        let query = `
    ...
    getRecentlyUsedBufferMediaIds(mediaType, hoursBack = 2) {
    ...
    getReusableMediaIds(mediaType, usageContext, excludeMostRecent = true) {
        const tableName = this.getTableName(mediaType);
        let query = `
      SELECT DISTINCT mediaItemId 
      FROM ${tableName} 
    ...
    `;
      ...
      `;
            ...
            const recentResult = recentStmt.get(...(usageContext ? [usageContext] : []));
            if (recentResult) {
    ...
    getMostRecentlyUsedMediaId(mediaType, usageContext) {
        const tableName = this.getTableName(mediaType);
        let query = `
      SELECT mediaItemId 
      FROM ${tableName} 
    ...
    getAvailableMediaStrategy(availableMediaIds, mediaType, usageContext) {
        const recentlyUsedIds = this.getRecentlyUsedMediaIds(mediaType, usageContext);
        const mostRecentId = this.getMostRecentlyUsedMediaId(mediaType, usageContext);
        // Strategy 1: Fresh media (not recently used)
        const freshMediaIds = availableMediaIds.filter(id => !recentlyUsedIds.includes(id));
        ...
        const availableReusableIds = availableMediaIds.filter(id => reusableIds.includes(id));
        ...
        const nonMostRecentIds = availableMediaIds.filter(id => id !== mostRecentId);
    ...
    filterAvailableMedia(availableMediaIds, mediaType, usageContext, allowReuse = true) {
        ...
        const strategy = this.getAvailableMediaStrategy(availableMediaIds, mediaType, usageContext);
    ...
    recordBatchUsage(mediaItems, streamSessionId) {
        ...
        `);
                for (const item of items) {
                    const usedAt = new Date();
                    const expiresAt = recentlyUsedMedia_1.RecentlyUsedMedia.getDefaultExpirationDate(item.usageContext);
                    stmt.run(item.mediaItemId, item.usageContext, streamSessionId || null, usedAt.toISOString(), expiresAt.toISOString());
                }
            }
        });
        transaction();
    ...
    getUsageByStreamSession(streamSessionId) {
        const tables = [
        ...
        ORDER BY usedAt DESC
      `);
            const rows = stmt.all(streamSessionId);
    ...
    clearUsageByTypeAndContext(mediaType, usageContext) {
        const tableName = this.getTableName(mediaType);
        const stmt = this.db.prepare(`
      DELETE FROM ${tableName} 
      WHERE usageContext = ?
    ...
    getUsageStats() {
        ...
        for (const table of tables) {
            // Count total records
            const totalStmt = this.db.prepare(`SELECT COUNT(*) as count FROM ${table.name}`);
      ...
      `);
            const expiredResult = expiredStmt.get();
      ...
      `);
            const contextResults = contextStmt.all();
    ...
    mapRowToRecentlyUsedMedia(row) {
        return new recentlyUsedMedia_1.RecentlyUsedMedia(row.mediaItemId, row.mediaType, row.usageContext, row.streamSessionId, new Date(row.usedAt), row.expiresAt ? new Date(row.expiresAt) : undefined, row.id);
    }
}
exports.RecentlyUsedMediaRepository = RecentlyUsedMediaRepository;
exports.recentlyUsedMediaRepository = new RecentlyUsedMediaRepository();
