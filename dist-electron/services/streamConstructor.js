 ...
 */
export function constructStream(streamType, rightNow, cadence = false, firstMedia) {
    switch (streamType) {
 ...
 */
function constructContinuousStream(rightNow, cadence = false, firstMedia) {
    const streamBlocks = [];
    try {
        ...
        const firstBlock = new MediaBlock([], // No buffer for first block
        selectedFirstMedia, currentTime);
        ...
        while (currentTime < endOfDay) {
            ...
            const mediaBlock = new MediaBlock([], nextMedia, currentTime);
...
function getNextEpisodeForShow(show, streamType) {
    ...
    if (currentEpisode) {
        ...
        const nextEpisodeNum = episodeNum < show.episodes.length ? episodeNum + 1 : 1; // Loop back to 1
        episodeProgressionRepository.updateEpisodeNumber(progression.episodeProgressionId, nextEpisodeNum, show.episodes.length);
    }
...
export function findMediaWithFacet(facet) {
    ...
    const moviesWithBoth = moviesWithGenre.filter((movie) => movie.tags.some((tag) => tag.tagId === facet.aesthetic.tagId));
    ...
    const showsWithBoth = showsWithGenre.filter((show) => show.tags.some((tag) => tag.tagId === facet.aesthetic.tagId));
