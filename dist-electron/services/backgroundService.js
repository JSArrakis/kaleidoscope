...
function validateUpcomingMediaFiles(currentUnixTimestamp) {
    ...
    for (const block of blocks) {
        if (block.startTime > validationEndTime)
            break;
        // Check anchor media file
        if (block.anchorMedia &&
            "path" in block.anchorMedia &&
            block.anchorMedia.path) {
            if (!existsSync(block.anchorMedia.path)) {
                console.warn(`[Media Validation] MISSING anchor file: "${block.anchorMedia.title}" ` +
        ...
        for (const item of block.buffer) {
            if ("path" in item && item.path) {
                if (!existsSync(item.path)) {
                    console.warn(`[Media Validation] MISSING buffer file: "${item.title}" ` +
                        `at path: ${item.path}`);
                    missingCount++;
                }
            }
    ...
    if (missingCount > 0) {
        console.warn(`[Media Validation] ${missingCount} missing file(s) in upcoming ${intervalInSeconds}s interval`);
    }
}
async function cycleCheck() {
    ...
    if (onDeck.length >= 2) {
        ...
        console.log("[Cycle Check] Not enough items in On Deck to check for a new item");
    }
    if (onDeck.length >= 1 && currentUnixTimestamp >= onDeck[0].startTime) {
        console.log(`[Cycle Check] "${onDeck[0].anchorMedia?.title ?? "(buffer block)"}" is starting now`);
    }
    ...
    if (streamManager.isActiveStream()) {
        while (onDeck.length > 1 && currentUnixTimestamp >= onDeck[1].startTime) {
            ...
            console.log(`[Cycle Check] Removed expired On Deck block: "${expired?.anchorMedia?.title ?? "(buffer block)"}"`);
        }
    }
    ...
    if (onDeck.length < 3) {
        ...
        if (upcoming.length > 0) {
            ...
            if (next) {
                ...
                console.log(`[Cycle Check] Promoted to On Deck Slot ${onDeck.length}: "${next.anchorMedia?.title ?? "(buffer block)"}"`);
            }
        }
    }
    // --- Day rollover ---
    ...
    const upcoming = streamManager.getUpcomingStream();
    const lastUpcoming = upcoming.length > 0 ? upcoming[upcoming.length - 1] : null;
    const shouldRollover = streamManager.isContinuousStream() &&
        lastUpcoming !== null &&
        upcoming.length === 1 &&
    ...
    if (shouldRollover) {
        ...
        if (args) {
            ...
            console.log("[Cycle Check] Day rollover triggered — next day's stream built");
        }
    }
    const shouldAdhocRollover = streamManager.isAdhocStream() &&
        lastUpcoming !== null &&
        upcoming.length === 1 &&
        lastUpcoming.buffer.length === 0 &&
    ...
    if (shouldAdhocRollover) {
        ...
        if (args) {
            ...
            rolloverAdhocToNextDay(options, tomorrow, streamManager.getAdhocStreamEndTimepoint());
            console.log("[Cycle Check] Adhoc day rollover triggered — next day's stream built");
        }
    }
