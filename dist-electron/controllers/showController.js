...
export async function createShow(show) {
    try {
        ...
        if (show.episodes && show.episodes.length > 0) {
            console.log(`[showController] Probing ${show.episodes.length} episode(s) for show: ${show.title}`);
            for (const episode of show.episodes) {
                ...
                console.log(`[showController] Episode ${episode.episodeNumber} probed: ${episode.duration} seconds, durationLimit: ${episode.durationLimit} seconds`);
            }
        }
...
export function getShow(mediaItemId) {
    if (!mediaItemId) {
...
export function updateShow(mediaItemId, updates) {
    try {
        if (!mediaItemId) {
            return { message: "Media Item ID is required", status: 400 };
        }
...
export function deleteShow(mediaItemId) {
    try {
        if (!mediaItemId) {
            return { message: "Media Item ID is required", status: 400 };
        }
