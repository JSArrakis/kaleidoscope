...
import { filterRecentlyUsedCommercials, filterRecentlyUsedMusic, filterRecentlyUsedShorts, } from "../services/streamConstruction/selectionHelpers.js";
...
function shuffleAndHalve(array) {
 ...
 */
export function isBufferMediaPoolValid(selectedCommercials, selectedShorts, selectedMusic, halfBufferDuration) {
    ...
    const validCommercials = selectedCommercials.filter((c) => (c.duration || 0) >= MIN_COMMERCIAL_DURATION);
    const validShorts = selectedShorts.filter((s) => (s.duration || 0) >= MIN_SHORT_OR_MUSIC_DURATION);
    const validMusic = selectedMusic.filter((m) => (m.duration || 0) >= MIN_SHORT_OR_MUSIC_DURATION);
    // Combine valid shorts and music
    const shortsAndMusic = [...validShorts, ...validMusic];
    const shortsAndMusicCount = shortsAndMusic.length;
    const shortsAndMusicTotalDuration = shortsAndMusic.reduce((sum, item) => sum + (item.duration || 0), 0);
    // Calculate usability score
    const avgDurationPerPiece = shortsAndMusicCount > 0
        ? shortsAndMusicTotalDuration / shortsAndMusicCount
        : 0;
    const slotsPerBuffer = avgDurationPerPiece > 0 ? Math.ceil(duration / avgDurationPerPiece) : 0;
    ...
    const shortsAndMusicUsabilityScore = totalSlotsNeeded > 0
        ? Math.min(shortsAndMusicCount / totalSlotsNeeded, 1.0)
        : shortsAndMusicCount > 0
            ? 1.0
            : 0;
    // Calculate usable duration
    const usableShortsAndMusicDuration = shortsAndMusicTotalDuration * shortsAndMusicUsabilityScore;
    // Calculate remaining needed from commercials
    const commercialsNeeded = TWO_HOUR_POOL_SECONDS - usableShortsAndMusicDuration;
    // Check if we have enough valid commercials
    const commercialsTotalDuration = validCommercials.reduce((sum, commercial) => sum + (commercial.duration || 0), 0);
    return commercialsTotalDuration >= commercialsNeeded;
}
export function selectBufferMedia(segmentedTags, activeHolidayTags, duration, shortOrMusicNumber, timepoint) {
    ...
    if (activeHolidayTags.length > 0) {
        const holidayResults = getHolidayBufferMedia(activeHolidayTags, ageAdjacencyTags, segmentedTags.specialtyTags, duration); // VERIFIED
        mergeUnique(selectedCommercials, filterRecentlyUsedCommercials(holidayResults.commercials, timepoint));
        mergeUnique(selectedShorts, filterRecentlyUsedShorts(holidayResults.shorts, timepoint));
        mergeUnique(selectedMusic, filterRecentlyUsedMusic(holidayResults.music, timepoint));
        if (isBufferMediaPoolValid(selectedCommercials, selectedShorts, selectedMusic, duration) // VERIFIED
        ) {
            return {
    ...
    if (segmentedTags.specialtyTags.length > 0) {
        const specialtyResults = getSpecialtyBufferMedia(segmentedTags.specialtyTags, duration); // VERIFIED
        mergeUnique(selectedCommercials, filterRecentlyUsedCommercials(specialtyResults.commercials, timepoint));
        mergeUnique(selectedShorts, filterRecentlyUsedShorts(specialtyResults.shorts, timepoint));
        mergeUnique(selectedMusic, filterRecentlyUsedMusic(specialtyResults.music, timepoint));
        if (isBufferMediaPoolValid(selectedCommercials, selectedShorts, selectedMusic, duration) // VERIFIED
        ) {
            return {
                commercials: selectedCommercials,
    ...
    if (segmentedTags.genreTags.length > 0 ||
        ...
        const genreAndAestheticResults = getGenreAndAestheticBufferMedia(segmentedTags.genreTags, segmentedTags.aestheticTags, ageAdjacencyTags, duration); // VERIFIED
        mergeUnique(selectedCommercials, filterRecentlyUsedCommercials(genreAndAestheticResults.commercials, timepoint));
        mergeUnique(selectedShorts, filterRecentlyUsedShorts(genreAndAestheticResults.shorts, timepoint));
        mergeUnique(selectedMusic, filterRecentlyUsedMusic(genreAndAestheticResults.music, timepoint));
        if (isBufferMediaPoolValid(selectedCommercials, selectedShorts, selectedMusic, duration) // VERIFIED
        ) {
            return {
                commercials: selectedCommercials,
    ...
    mergeUnique(selectedCommercials, filterRecentlyUsedCommercials(ageOnlyResults.commercials, timepoint));
    mergeUnique(selectedShorts, filterRecentlyUsedShorts(ageOnlyResults.shorts, timepoint));
    mergeUnique(selectedMusic, filterRecentlyUsedMusic(ageOnlyResults.music, timepoint));
    if (isBufferMediaPoolValid(selectedCommercials, selectedShorts, selectedMusic, duration) // VERIFIED
    ) {
        return {
            commercials: selectedCommercials,
    ...
    mergeUnique(selectedCommercials, filterRecentlyUsedCommercials(untaggedResults.commercials, timepoint));
    mergeUnique(selectedShorts, filterRecentlyUsedShorts(untaggedResults.shorts, timepoint));
    mergeUnique(selectedMusic, filterRecentlyUsedMusic(untaggedResults.music, timepoint));
    if (segmentedTags.ageGroupTags.length === 0 &&
        segmentedTags.specialtyTags.length === 0 &&
        segmentedTags.genreTags.length === 0 &&
        segmentedTags.aestheticTags.length === 0) {
        const randomBufferMedia = getRandomBufferMedia(duration, shortOrMusicNumber);
        mergeUnique(selectedCommercials, filterRecentlyUsedCommercials(randomBufferMedia.commercials, timepoint));
        mergeUnique(selectedShorts, filterRecentlyUsedShorts(randomBufferMedia.shorts, timepoint));
        mergeUnique(selectedMusic, filterRecentlyUsedMusic(randomBufferMedia.music, timepoint));
    }
    const isValid = isBufferMediaPoolValid(selectedCommercials, selectedShorts, selectedMusic, duration); // VERIFIED
...
function getAgeGroups(ageGroupTags) {
    ...
    const sortedAgeGroups = ageGroupTags.sort((a, b) => (a.sequence || 0) - (b.sequence || 0));
    const baseAgeGroup = sortedAgeGroups[0];
    let adjacencyTags = [baseAgeGroup];
...
export function getHolidayBufferMedia(holidayTags, ageGroupTags, specialtyTags, duration) {
    // Get all commercials that match holiday and age group tags from the DB
    const commercials = commercialRepository.findHolidayCommercials(ageGroupTags, holidayTags, specialtyTags, duration);
    const shorts = shortRepository.findHolidayShorts(ageGroupTags, holidayTags, specialtyTags, duration);
    const music = musicRepository.findHolidayMusic(ageGroupTags, holidayTags, specialtyTags, duration);
    return {
        commercials,
        shorts,
        music,
...
export function getSpecialtyBufferMedia(specialtyTags, duration) {
    // Get all commercials that match specialty tags from the DB
    const commercials = commercialRepository.findBySpecialtyTags(specialtyTags, duration);
...
export function getGenreAndAestheticBufferMedia(genreTags, aestheticTags, ageGroupTags, duration) {
    // Get all commercials that match genre/aesthetic and age group tags from the DB
    const commercials = commercialRepository.findByGenreAestheticAgeGroup(genreTags, aestheticTags, ageGroupTags, duration);
    const shorts = shortRepository.findByGenreAestheticAgeGroup(genreTags, aestheticTags, ageGroupTags, duration);
...
export function getAgeGroupOnlyBufferMedia(ageGroupTags, duration) {
    // Get all commercials that match age group tags from the DB
    const commercials = commercialRepository.findByAgeGroupOnly(ageGroupTags, duration);
...
export function getUntaggedBufferMedia(duration) {
...
export function getRandomBufferMedia(duration, shortOrMusicNumber) {
    // Get up to 2 hours of random commercials under the passed duration
    const commercials = commercialRepository.findRandomCommercialsByPoolDuration(duration);
    // Get random shorts and music videos with coin-flip distribution
    // Limited by count and total duration
    const shortsAndMusic = shortRepository.findRandomShortsAndMusicByCount(shortOrMusicNumber, duration);
    return {
        commercials,
