...
import { getDateString, getProgressionsByStreamType, isHolidayDate, isHolidaySeason, } from "./selectionHelpers.js";
 ...
 */
export async function buildContinuousStream(streamConstructionOptions, startTimepoint) {
    const streamBlocks = [];
    const constructionStartTime = Date.now();
    let mediaBlocks = [];
    try {
        // Initialize stream data
        const initData = initializeContinuousStream(startTimepoint, streamConstructionOptions); // VERIFIED
        ...
        const todayIsHolidayDate = isHolidayDate(startTimepoint, initData.activeHolidayTags); // VERIFIED
        const todayIsHolidaySeason = isHolidaySeason(startTimepoint, initData.activeHolidayTags); // VERIFIED
        if (streamConstructionOptions.Cadence) {
            // CADENCED MODE: Use buffers and align to :00/:30 marks
            mediaBlocks = await buildCadencedContinuousStream(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, constructionStartTime);
        }
        else {
            return [[], "Uncadenced continuous streams not yet implemented"];
        }
        streamBlocks.push(...mediaBlocks);
        console.log(`[ContinuousStreamBuilder] Created ${streamBlocks.length} media blocks`);
        return [streamBlocks, ""];
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        console.error(`[ContinuousStreamBuilder] Stream construction failed: ${message}`);
        return [[], message];
    }
}
/**
 ...
 */
function initializeContinuousStream(incomingTimepoint, streamConstructionOptions) {
    const fullDateString = new Date(incomingTimepoint * 1000)
        .toISOString()
        .substring(0, 10); // Get only YYYY-MM-DD
    // Load recently used media from DB into stream manager
    loadRecentlyUsedMedia(incomingTimepoint); // VERIFIED
    const activeHolidayTags = tagRepository.findActiveHolidaysByDate(fullDateString); // VERIFIED
    ...
    const iterationDuration = Math.floor((endOfTimeWindow - incomingTimepoint) / (30 * 60)) * 30 * 60;
    // Get progression map for this stream type and set it on the stream manager
    const progressionMap = getProgressionsByStreamType(streamConstructionOptions.StreamType); // VERIFIED
    streamManager.getStreamManager().setProgressionMap(progressionMap); // VERIFIED
    // Select first random media as fallback
    const selectedFirstMedia = selectRandomShowOrMovie(incomingTimepoint, iterationDuration, []); // VERIFIED
    return {
        activeHolidayTags,
        progressionMap,
...
/**
 * Loads recently used media from the database into the stream manager
 * Filters each media type by a recency window to avoid repeating content too soon
 *
 * Recency windows (configurable?):
 * - Movies: 48 hours
 * - Commercials: 3 hours
 * - Shorts: 24 hours
 * - Music: 24 hours
 */
function loadRecentlyUsedMedia(incomingTimepoint) {
    const recentlyUsedMedia = recentlyUsedMediaRepository.findAll();
    const recencyWindows = {
        [MediaType.Movie]: { hours: 48, addFn: streamManager.addRecentlyUsedMovie },
        [MediaType.Commercial]: {
            hours: 3,
            addFn: streamManager.addRecentlyUsedCommercial,
        },
        [MediaType.Short]: { hours: 24, addFn: streamManager.addRecentlyUsedShort },
        [MediaType.Music]: { hours: 24, addFn: streamManager.addRecentlyUsedMusic },
    };
    for (const media of recentlyUsedMedia) {
        const config = recencyWindows[media.mediaType];
        if (!config)
            continue;
        if (media.lastUsedDate) {
            const unixTime = Math.floor(new Date(media.lastUsedDate).getTime() / 1000);
            if (unixTime >= incomingTimepoint - config.hours * 3600) {
                config.addFn(media.mediaItemId, unixTime);
            }
        }
    }
}
 ...
 */
async function buildCadencedContinuousStream(streamConstructionOptions, initData, dateString, // Date without time (YYYY-MM-DD) for holiday tag matching
todayIsHolidayDate, todayIsHolidaySeason, constructionStartTime) {
    const streamBlocks = [];
    ...
    if (nextCadenceTime > initData.startingTimepoint) {
        ...
        streamBlocks.push(...buildCadencedWithInitialBuffer(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, nextCadenceTime, constructionStartTime));
    }
    else {
        // CASE 2: No initial buffer needed
        streamBlocks.push(...buildCadencedWithoutInitialBuffer(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, constructionStartTime));
    }
    return streamBlocks;
}
/**
 ...
 */
function buildCadencedWithInitialBuffer(streamConstructionOptions, initData, dateString, // Date without time (YYYY-MM-DD) for holiday tag matching
todayIsHolidayDate, todayIsHolidaySeason, nextCadenceTime, constructionStartTime) {
    const streamBlocks = [];
    // Defensive check: ensure selectedFirstMedia exists
    if (!initData.selectedFirstMedia) {
        console.error("[ContinuousStreamBuilder] No media available for stream construction");
        return streamBlocks;
    }
    ...
    if (initialBufferDuration > 0) {
        ...
        const initialBufferResult = createBuffer(initialBufferDuration, [], firstMediaTags, initData.activeHolidayTags, initData.startingTimepoint);
        const initialBufferBlock = createMediaBlock(initialBufferResult.buffer, undefined, // No mainBlock for initial buffer
    ...
    const firstAnchorMediaBlock = createMediaBlock([], initData.selectedFirstMedia, nextCadenceTime);
    ...
    const [remainingStreamBlocks] = selectSecondAnchorAndBuildStream(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, firstAnchorMediaBlock, nextCadenceTime);
 ...
 */
function buildCadencedWithoutInitialBuffer(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, constructionStartTime) {
    const streamBlocks = [];
    // Defensive check: ensure selectedFirstMedia exists
    if (!initData.selectedFirstMedia) {
        console.error("[ContinuousStreamBuilder] No media available for stream construction");
    ...
    const firstAnchorMediaBlock = createMediaBlock([], initData.selectedFirstMedia, initData.startingTimepoint);
    ...
    const [remainingStreamBlocks] = selectSecondAnchorAndBuildStream(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, firstAnchorMediaBlock, initData.startingTimepoint);
    streamBlocks.push(...remainingStreamBlocks);
    return streamBlocks;
}
/**
 ...
 */
function selectSecondAnchorAndBuildStream(streamConstructionOptions, initData, dateString, todayIsHolidayDate, todayIsHolidaySeason, firstAnchorMediaBlock, firstAnchorTimepoint) {
    const streamBlocks = [];
    // Defensive check: ensure selectedFirstMedia exists
    if (!initData.selectedFirstMedia) {
        console.error("[ContinuousStreamBuilder] Cannot build stream: no first media selected");
    ...
    const secondAnchorMedia = streamConstructionOptions.Themed
        ? selectThemedMedia(segmentedTags, firstAnchorTimepoint, initData.iterationDuration, initData.progressionMap, initData.activeHolidayTags, todayIsHolidayDate, todayIsHolidaySeason, dateString)
        : selectRandomShowOrMovie(firstAnchorTimepoint, initData.iterationDuration, segmentedTags.ageGroupTags);
    if (!secondAnchorMedia) {
        console.warn("[ContinuousStreamBuilder] Could not select second anchor media");
        return [streamBlocks, null];
    }
    // Create second anchor media block (for buffer calculation)
    const secondAnchorMediaBlock = createMediaBlock([], secondAnchorMedia, firstAnchorTimepoint + firstAnchorMediaBlock.duration);
    // Create backfill buffer between first and second anchors
    const backfillBuffer = createBackfillBuffer(firstAnchorMediaBlock, secondAnchorMediaBlock, initData.activeHolidayTags);
    ...
    const remainingStreamBlocks = constructContinuousStreamBlocks(firstAnchorTimepoint + firstAnchorMediaBlock.duration, initData.endOfTimeWindow, initData.selectedFirstMedia.tags || [], initData.activeHolidayTags, initData.progressionMap, streamConstructionOptions, secondAnchorMediaBlock, dateString, todayIsHolidayDate, todayIsHolidaySeason);
    streamBlocks.push(...remainingStreamBlocks);
 ...
 */
function createBackfillBuffer(mediaBlockA, mediaBlockB, activeHolidayTags) {
    let backfillDuration = 0;
    if (mediaBlockA.mainBlock &&
    ...
    const backfillBufferResult = createBuffer(backfillDuration, tagsA, tagsB, activeHolidayTags, mediaBlockA.startTime);
 ...
 */
function constructContinuousStreamBlocks(incomingTimepoint, endOfTimeWindow, firstMediaTags, activeHolidayTags, progressionMap, streamConstructionOptions, iterationFirstMediaBlock, dateString, todayIsHolidayDate, todayIsHolidaySeason) {
    ...
    while (timepoint < endOfTimeWindow) {
        ...
        const nextMedia = streamConstructionOptions.Themed
            ? selectThemedMedia(segmentedTags, timepoint, remainingDuration, progressionMap, activeHolidayTags, todayIsHolidayDate, todayIsHolidaySeason, dateString)
            : selectRandomShowOrMovie(timepoint, remainingDuration, segmentedTags.ageGroupTags);
        if (!nextMedia) {
            console.warn(`[ContinuousStreamBuilder] Could not select media at timepoint ${timepoint}`);
            break;
        }
        const block = createMediaBlock([], nextMedia, timepoint);
