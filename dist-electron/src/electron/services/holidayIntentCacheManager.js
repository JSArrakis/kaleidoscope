...
class HolidayIntentCacheManager {
     ...
     */
    calculateIntent(holidayTagId) {
        // Get total runtime of all holiday content (movies + shows)
        // TODO: Implement getTotalMinutesByHolidayTag() in repositories
        const movieMinutes = movieRepository.getTotalMinutesByHolidayTag(holidayTagId);
        const showMinutes = showRepository.getTotalMinutesByHolidayTag(holidayTagId);
        const totalAvailableMinutes = movieMinutes + showMinutes;
        // Calculate 3-day distribution
        const threeDayDistribution = this.calculateThreeDayDistribution(totalAvailableMinutes);
     ...
     */
    calculateThreeDayDistribution(totalMinutes) {
     ...
     */
    getTodayTargetMinutes(holidayTagId, dateString) {
        const intent = this.getIntent(holidayTagId);
        // Check if we've moved to a new day
        if (intent.lastResetDate !== dateString) {
            // Update rotation day
            intent.currentRotationDay = this.getRotationDay(dateString);
     ...
     */
    canAddMoreContent(holidayTagId, dateString) {
        const intent = this.getIntent(holidayTagId);
        ...
        const targetToday = intent.threeDayDistribution[intent.currentRotationDay - 1];
     ...
     * @param dateString ISO date string
     */
    trackSelectedMinutes(holidayTagId, minutes, dateString) {
        const intent = this.getIntent(holidayTagId);
     ...
     */
    getRemainingMinutesToday(holidayTagId, dateString) {
        const intent = this.getIntent(holidayTagId);
        // Update if needed
        if (intent.lastResetDate !== dateString) {
            intent.currentRotationDay = this.getRotationDay(dateString);
     ...
     */
    invalidateHoliday(holidayTagId) {
        const cached = this.cache.get(holidayTagId);
        if (cached) {
            cached.stale = true;
        }
