 ...
 */
export async function createStream(streamType, streamConstructionOptions, rightNow) {
    switch (streamType) {
 ...
 */
async function constructContinuousStream(streamConstructionOptions, incomingTimepoint) {
    const streamBlocks = [];
    const constructionStartTime = Date.now(); // Capture start time for deviation tracking
    try {
        // Create initialization data
        let initializationData = createInitializationData(incomingTimepoint, streamConstructionOptions);
        ...
        if (streamConstructionOptions.Cadence) {
            ...
            const nextCadenceTime = findNextCadenceTime(initializationData.startingTimepoint);
            if (nextCadenceTime > initializationData.startingTimepoint) {
                ...
                const fillerDuration = nextCadenceTime - initializationData.startingTimepoint;
                // Create buffer using tags from previous block or selected first media
                const initialBuffer = createBuffer(fillerDuration, [], initializationData.selectedFirstMedia?.tags || [], initializationData.activeHolidayTags, initializationData.startingTimepoint);
                // Create media block for initial buffer
                const initialBufferBlock = createMediaBlock(initialBuffer.buffer, undefined, initializationData.startingTimepoint);
                ...
                const initialAnchorMediaBlock = createMediaBlock([], initializationData.selectedFirstMedia, nextCadenceTime);
                await playerManager.addMediaBlockToPlayer(initialAnchorMediaBlock);
                // Segment tags for themed selection
                const segmentedTags = segmentTags(initializationData.selectedFirstMedia?.tags || []);
                // Determine if today is a holiday date or within a holiday season
                const todayIsHolidayDate = isHolidayDate(incomingTimepoint, initializationData.activeHolidayTags);
                const todayIsHolidaySeason = isHolidaySeason(incomingTimepoint, initializationData.activeHolidayTags);
                ...
                const firstIterationAnchorMedia = streamConstructionOptions.Themed
                    ? selectThemedMediaForStream(segmentedTags, initializationData.startingTimepoint, initializationData.iterationDuration, initializationData.progressionMap, initializationData.activeHolidayTags, todayIsHolidayDate, todayIsHolidaySeason)
                    : selectRandomMediaForStream(initializationData.startingTimepoint, initializationData.iterationDuration, initializationData.progressionMap, []);
                // Create media block for first media of iteration
                const iterationFirstMediaBlock = createMediaBlock([], firstIterationAnchorMedia, nextCadenceTime + initialAnchorMediaBlock.duration);
                ...
                await createAndPushBackfillBuffer(initialAnchorMediaBlock, iterationFirstMediaBlock, initializationData.activeHolidayTags);
                ...
                const iterationTimePoint = nextCadenceTime + initializationData.selectedFirstMedia.durationLimit;
                // Construct remaining stream blocks until the end of the day
                const remainingStreamBlocks = constructStream(iterationTimePoint, initializationData.endOfTimeWindow, initializationData.selectedFirstMedia.tags || [], initializationData.activeHolidayTags, streamConstructionOptions, iterationFirstMediaBlock, initializationData.progressionMap);
                ...
                // ============================================================================
                const initialBufferBlock = createMediaBlock([], initializationData.selectedFirstMedia, initializationData.startingTimepoint);
                ...
                // Segment tags and determine holiday context for selection
                const segmentedTags = segmentTags(initializationData.selectedFirstMedia?.tags || []);
                const todayIsHolidayDate = isHolidayDate(incomingTimepoint, initializationData.activeHolidayTags);
                const todayIsHolidaySeason = isHolidaySeason(incomingTimepoint, initializationData.activeHolidayTags);
                // Select first media for remaining stream
                const firstIterationAnchorMedia = streamConstructionOptions.Themed
                    ? selectThemedMediaForStream(segmentedTags, initializationData.startingTimepoint, initializationData.iterationDuration, initializationData.progressionMap, initializationData.activeHolidayTags, todayIsHolidayDate, todayIsHolidaySeason)
                    : selectRandomMediaForStream(initializationData.startingTimepoint, initializationData.iterationDuration, initializationData.progressionMap, []);
                const firstIterationAnchorMediaBlock = createMediaBlock([], firstIterationAnchorMedia, initializationData.startingTimepoint);
                // Construct remaining stream blocks
                const remainingStreamBlocks = constructStream(initializationData.startingTimepoint, initializationData.endOfTimeWindow, initializationData.selectedFirstMedia.tags || [], initializationData.activeHolidayTags, streamConstructionOptions, firstIterationAnchorMediaBlock, initializationData.progressionMap);
                // Create backfill buffer between firstBlock and first remaining block
                if (remainingStreamBlocks.length > 0) {
                    await createAndPushBackfillBuffer(initialBufferBlock, remainingStreamBlocks[0], initializationData.activeHolidayTags);
                }
                // Add remaining stream blocks to streamBlocks
                streamBlocks.push(...remainingStreamBlocks);
            }
            ...
            // ============================================================================
            // CADENCE: FALSE (No buffers created, only shows/movies)
            // ============================================================================
            const initialBufferBlock = createMediaBlock([], initializationData.selectedFirstMedia, initializationData.startingTimepoint);
            // Push firstBlock immediately to player
            const timeDeltaMs = Date.now() - constructionStartTime;
            ...
            // Construct remaining stream blocks
            const remainingStreamBlocks = constructStream(initializationData.startingTimepoint, initializationData.endOfTimeWindow, initializationData.selectedFirstMedia.tags || [], initializationData.activeHolidayTags, streamConstructionOptions, firstIterationAnchorMediaBlock, initializationData.progressionMap);
            // NOTE: No backfill buffers created in non-cadence mode
            // Only shows and movies are selected
            // Add remaining stream blocks to streamBlocks
            streamBlocks.push(...remainingStreamBlocks);
        }
        ...
        console.log(`[StreamConstructor] Created ${streamBlocks.length} media blocks for continuous stream`);
        return [streamBlocks, ""];
    }
    catch (err) {
...
// HELPER FUNCTIONS
// ============================================================================
function createInitializationData(incomingTimepoint, streamConstructionOptions) {
    ...
    const activeHolidayTags = tagRepository.findActiveHolidaysByDate(fullDateString);
    ...
    let progressionMap = getProgressionsByStreamType(streamConstructionOptions.StreamType);
    const selectedFirstMedia = selectRandomMediaForStream(incomingTimepoint, iterationDuration, progressionMap, []);
    return {
 ...
 */
function isHolidayDate(unixSeconds, holidays) {
 ...
 */
function isHolidaySeason(incomingTimepoint, holidays) {
 ...
 */
function getProgressionsByStreamType(streamType) {
 ...
 */
function doesNextEpisodeFitDuration(show, progressionMap, availableDuration) {
 ...
 */
function incrementShowProgression(show, progressionMap) {
 ...
 */
function constructStream(incomingTimepoint, endOfTimeWindow, firstMediaTags, activeHolidayTags, streamConstructionOptions, iterationFirstMediaBlock, progressionMap) {
    ...
    // Determine if today is a holiday date or within a holiday season
    const todayIsHolidayDate = isHolidayDate(incomingTimepoint, activeHolidayTags);
    const todayIsHolidaySeason = !todayIsHolidayDate &&
    ...
    while (timepoint < endOfTimeWindow) {
        ...
        const nextMedia = streamConstructionOptions.Themed
            ? selectThemedMediaForStream(segmentedTags, timepoint, remainingDuration, progressionMap, activeHolidayTags, todayIsHolidayDate, todayIsHolidaySeason)
            : selectRandomMediaForStream(timepoint, remainingDuration, progressionMap, []);
 ...
 */
async function createAndPushBackfillBuffer(mediaBlockA, mediaBlockB, activeHolidayTags) {
    ...
    if (backfillDuration > 0) {
        ...
        const backfillBuffer = createBuffer(backfillDuration, tagsA, tagsB, activeHolidayTags, mediaBlockA.startTime);
 ...
 */
function selectRandomMediaForStream(timepoint, duration, progressionMap, ageGroups) {
    ...
    if (chooseMovie) {
        // Select random movie
        const movie = movieRepository.findRandomMovieUnderDuration(duration, ageGroups);
        if (movie) {
            return movie;
        }
        else {
...
export function getEpisodeFromShowCandidates(shows, progressionMap, duration) {
 ...
 */
function selectHolidayMediaForTag(holidayTagId, dateString) {
 ...
 */
function selectSpecialtyAdjacentMedia(specialtyTags, ageGroupTags, duration, progressionMap) {
...
export function selectThemedMediaForStream(segmentedTags, timepoint, duration, progressionMap, activeHolidayTags, isHolidayDate, isHolidaySeason) {
    ...
    if (isHolidaySeason) {
        ...
        for (const holidayTag of activeHolidayTags) {
            ...
            if (holidayIntentCacheManager.canAddMoreContent(holidayTag.tagId, dateString)) {
                ...
                const selectedMedia = selectHolidayMediaForTag(holidayTag.tagId, dateString);
                if (selectedMedia) {
    ...
    if (segmentedTags.specialtyTags.length > 0 && Math.random() < 0.5) {
        const selectedMedia = selectSpecialtyAdjacentMedia(segmentedTags.specialtyTags, duration, progressionMap);
        if (selectedMedia) {
            return selectedMedia;
        }
    }
    ...
    if (!selectedMedia) {
        return selectRandomMediaForStream(timepoint, duration, progressionMap, segmentedTags.ageGroupTags);
    }
    return selectedMedia;
}
/**
 ...
 */
function getNextEpisodeForShow(show, streamType) {
    ...
    let progression = episodeProgressionRepository.findByShowAndStreamType(show.mediaItemId, streamType);
    if (!progression) {
...
export function tempFunctionName() {
}
