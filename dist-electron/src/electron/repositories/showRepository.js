...
export class ShowRepository {
    ...
    findAllShowsUnderDuration(maxDuration, ageGroups = []) {
      ...
      `;
        ...
        const showRows = stmt.all(params);
        const shows = [];
        for (const showRow of showRows) {
            const episodesStmt = this.db.prepare(`
        SELECT * FROM episodes WHERE showId = ? ORDER BY episodeNumber, episode
     ...
     */
    findRandomShowUnderDuration(maxDuration) {
        const stmt = this.db.prepare(`
      SELECT * FROM shows 
      WHERE durationLimit <= ?
      ORDER BY RANDOM() LIMIT 1
    `);
        const showRow = stmt.get(maxDuration);
        if (!showRow)
            return null;
        const episodesStmt = this.db.prepare(`
      SELECT * FROM episodes WHERE showId = ? ORDER BY episodeNumber, episode
    ...
    `);
      ...
      `);
            const episodeRows = episodesStmt.all(showRow.id, maxDuration);
     ...
     */
    findByTagsAndDurationLimit(tagIds, maxDurationLimit, ageGroups = []) {
        if (tagIds.length === 0)
            return [];
        const ageGroupIds = ageGroups.map((ageGroup) => ageGroup.tagId);
    ...
    `;
        const params = [maxDurationLimit, ...tagIds, tagIds.length];
        if (ageGroupIds.length > 0) {
            query += `
        AND s.mediaItemId IN (
          SELECT mediaItemId FROM show_tags 
     ...
     */
    findByTagsAndAgeGroupsUnderDuration(tags, ageGroups = [], maxDurationLimit) {
        if (tags.length === 0)
            return [];
     ...
     */
    insertShowSecondaryTags(mediaItemId, tagIds) {
        if (tagIds.length === 0)
            return;
        const stmt = this.db.prepare(`
    ...
    `);
      ...
      WHERE s.mediaItemId = ?
    `);
        const episodeTagRows = episodeTagsStmt.all(mediaItemId);
        const episodeTagIds = new Set(episodeTagRows.map((r) => r.tagId));
        // Compute secondary tags (episode tags NOT in primary tags)
        const secondaryTagIds = Array.from(episodeTagIds).filter((tagId) => !primaryTagIds.has(tagId));
    ...
    `);
      ...
      ORDER BY t.name
    `);
        const secondaryTagRows = secondaryTagsStmt.all(showRow.mediaItemId);
    ...
    findByEpisodeTags(tagIds) {
      ...
      ORDER BY s.title
    `);
        const showRows = showStmt.all(...tagIds);
        return showRows.map((showRow) => {
            const episodeStmt = this.db.prepare(`
        SELECT * FROM episodes WHERE showId = ?
      `);
            const episodeRows = episodeStmt.all(showRow.id);
            return this.mapRowToShow(showRow, episodeRows);
        });
    }
    /**
     ...
     */
    findByIdsWithProgressionCheck(showIds, tempProgressions, streamType, availableDuration) {
        if (showIds.length === 0)
            return [];
        // Get all shows first
    ...
    `);
        const showRows = showStmt.all(...showIds);
        const validShows = [];
        for (const showRow of showRows) {
            const episodeStmt = this.db.prepare(`
        SELECT * FROM episodes WHERE showId = ?
     ...
     * @returns Array of shows where the next episode will fit in the available duration
     */
    findByTagsAndDurationWithProgressionCheck(tagIds, streamType, availableDuration) {
        if (tagIds.length === 0)
            return [];
        const placeholders = tagIds.map(() => "?").join(",");
      ...
      ORDER BY s.title
    `);
        const showRows = showStmt.all(...tagIds, availableDuration);
        const validShows = [];
        for (const showRow of showRows) {
      ...
      `);
            const progression = progressionStmt.get(showRow.mediaItemId, streamType);
      ...
      `);
            const nextEpisodeRow = episodeStmt.get(showRow.id, nextEpisodeNumber);
            if (nextEpisodeRow &&
          ...
          SELECT * FROM episodes WHERE showId = ?
        `);
                const episodeRows = allEpisodeStmt.all(showRow.id);
                validShows.push(this.mapRowToShow(showRow, episodeRows));
            }
        }
        return validShows;
