...
export class CollectionRepository {
    ...
    create(collection) {
        ...
        VALUES (?, ?, ?, ?)
      `);
            collectionStmt.run(collection.collectionId, collection.title, collection.description || null, collection.itemCount);
        ...
        `);
                for (const item of collection.items) {
                    itemStmt.run(item.collectionItemId, collection.collectionId, item.mediaItemId, item.sequence);
                }
            }
        });
        transaction();
    ...
    findByCollectionId(collectionId) {
        const stmt = this.db.prepare(`SELECT * FROM collections WHERE collectionId = ?`);
    ...
    addItemToCollection(collectionItemId, collectionId, mediaItemId, sequence) {
        const transaction = this.db.transaction(() => {
            const stmt = this.db.prepare(`
        INSERT INTO collection_items (collectionItemId, collectionId, mediaItemId, sequence)
        VALUES (?, ?, ?, ?)
      `);
            ...
            // Update item count
            this.db
                .prepare(`UPDATE collections SET itemCount = itemCount + 1 WHERE collectionId = ?`)
                .run(collectionId);
        });
        transaction();
    ...
    removeItemFromCollection(collectionItemId) {
        const transaction = this.db.transaction(() => {
            const itemStmt = this.db.prepare(`SELECT collectionId FROM collection_items WHERE collectionItemId = ?`);
            const item = itemStmt.get(collectionItemId);
            if (!item)
                return false;
            const deleteStmt = this.db.prepare(`DELETE FROM collection_items WHERE collectionItemId = ?`);
    ...
    updateCollectionItem(collectionItemId, sequence) {
        const stmt = this.db.prepare(`
    ...
    deleteCollection(collectionId) {
        const transaction = this.db.transaction(() => {
            this.db
                ...
                .run(collectionId);
            const stmt = this.db.prepare(`DELETE FROM collections WHERE collectionId = ?`);
    ...
    findCollectionItemById(collectionItemId) {
        const stmt = this.db.prepare(`SELECT * FROM collection_items WHERE collectionItemId = ?`);
        const row = stmt.get(collectionItemId);
        if (!row)
            return null;
